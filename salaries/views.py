...
def showLevel_SalariesList(request):
    ...
    if keyword:
        ...
        level_salaries = level_salaries.filter(Q(basicSalary__icontains=keyword.split('.')[0]) | Q(
            coefficientPay__icontains=keyword) | Q(coefficientAllowance__icontains=keyword))
...
def saveLevel_Salary(request):
    if request.method == "POST":
        ...
        if id == 0:
            salary = Level_Salary(
                basicSalary=basicSalary, coefficientPay=coefficientPay, coefficientAllowance=coefficientAllowance)
            salary.save()
