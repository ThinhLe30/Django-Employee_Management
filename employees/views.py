...
def saveEmployee(request):
    if request.method == "POST":
        ...
        if id == 0:
            employee = Employee(firstName=firstName, lastName=lastname, email=email,birthday=birthday,address=address,gender=gender, phone=phone,image=image_file.name if image_file else 'default.jpg',department_id=id_department, salary_id=id_salary)
            employee.save()
